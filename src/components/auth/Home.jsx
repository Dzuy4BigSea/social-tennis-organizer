...
export default function Home({ onStartTournament }) {
  const { profile, user, currentOrg, signOut } = useAuth()
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-emerald-700 text-white px-4 py-3 flex items-center gap-4 shadow">
        <div className="flex-1">
          <h1 className="font-bold text-lg">
            {currentOrg?.name ?? '🎾 Tennis Organizer'}
          </h1>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        <div className="rounded-2xl p-6 border border-amber-200 bg-amber-50 text-sm text-amber-800">
          <strong>Account ready.</strong> Cloud-saved events, the
          members panel, and invite codes arrive in the next steps.
          For now, your tournaments still save locally.
        </div>
        <button
